#
# Train two VGG16 models which are coupled and we are trying to minimize the contrastive loss between the
...
from keras_tools import build_coupled_vgg_model, coupled_image_loader, contrastive_loss
...
learning_rate_c = 0.1         # Learning rate of the coupled layer (contrastive loss)
...
model = build_coupled_vgg_model(weight_path=None, nb_class=nb_class, nb_fc=[512, 512], coupled_layer='flatten')
...
model.compile(optimizer=sgd,
                   loss={"predictions_l": 'categorical_crossentropy', "predictions_r": 'categorical_crossentropy', 'coupled_loss': contrastive_loss},
...
for e in range(nb_epoch):
    ...
    for X_train, Y_train in coupled_image_loader(batch_size, hdf5_path, nb_class, coupled_loss_type='contrastive'):
